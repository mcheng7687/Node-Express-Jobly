...
describe("GET /users/:username", function () {
  test("works for current user accessing their info", async function () {
    const resp = await request(app)
        .get(`/users/u1`)
        .set("authorization", `Bearer ${u1Token}`);
    expect(resp.body).toEqual({
...
describe("PATCH /users/:username", () => {
  test("works for current users updating their info", async function () {
    const resp = await request(app)
        .patch(`/users/u1`)
        .send({
          firstName: "New",
  ...
  test("works for admin users updating users' info", async function () {
    const resp = await request(app)
        .patch(`/users/u1`)
        .send({
          firstName: "New",
  ...
  test("works by current user: set new password", async function () {
    const resp = await request(app)
        .patch(`/users/u1`)
        .send({
  ...
  test("works by admin user: set new password", async function () {
    const resp = await request(app)
        .patch(`/users/u1`)
        .send({
          password: "new-password",
