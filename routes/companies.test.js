...
describe("GET /companies", function () {
  ...
  test("ok for anon for searching", async function () {
    const resp = await request(app).get("/companies?minEmployees=2&nameLike=c2");
    expect(resp.body).toEqual({
      companies:
          [
            {
  ...
  test("bad request when min employees > max employees in query", async function () {
    // bad request if query string has min employees > max employees
